"""
USA Market Bot Scanner
VCP-based scanning for S&P 500 stocks
"""
import os
import sys
import json
...
def get_vcp_signals(min_score=60):
    """Scan USA stocks using VCP logic from backend"""
    from engine import fetch_data, compute_indicators, DETECTOR
    
    signals = []
    symbols = list(CONFIG["symbols"].keys())[:50]  # Limit to 50 for speed
    
    print(f"Scanning {len(symbols)} USA stocks...")
    
    for symbol in symbols:
        try:
            df = fetch_data(symbol, period="1y", market="US")
...
if __name__ == "__main__":
    ...
    for s in signals[:10]:
        print(f"  {s['symbol']:8} | VCP: {s['vcp_score']:.0f} | RSI: {s['rsi']:.0f} | Score: {s['score']}")
