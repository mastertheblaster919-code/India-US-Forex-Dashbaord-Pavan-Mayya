"""
FOREX Bot Scheduler
Auto-runs scanner every 2 hours
"""
import schedule
import time
import logging
from datetime import datetime
from scanner_v2 import get_signals_above_threshold, log_signal
...
def run_portfolio_check():
    ...
    try:
        from scanner_v2 import scan_all
...
def start_scheduler():
    ...
    logger.info(f"Starting FOREX Bot Scheduler - 2H Timeframe")
    logger.info(f"Scan interval: every {interval} hours")
    logger.info(f"Instruments: {len(CONFIG['symbols'])}")
