...
Global Swing Command Center for FOREX
"""
import sys
import argparse
from datetime import datetime

from scanner import get_signals_above_threshold, scan_all, log_signal
...
def run_scan():
    ...
    print(f"\n{'='*60}")
    print(f"FOREX SCANNER - {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'='*60}")
    ...
    for s in signals:
        ...
        print(f"  {s['name']:10} | {dir_emoji:5} | Score: {s['score']:5.0f} | Price: {s['price']:.4f}")
...
def main():
    ...
    parser.add_argument("command", choices=["scan", "status", "journal", "help"], 
                        help="Command to run")
...
FOREX Trading Bot Commands:
  python main.py scan    - Run scanner and process signals
